...
function showModal() {
  ...
  const forms = {
    ...
    share: `<h2>Un enlace para el plan</h2><p>Guarda este enlace para volver a tu evento.</p><label>Enlace del evento<input id="share-link" readonly value="${esc(location.href)}"></label><div class="warning">Por ahora los datos se guardan solo en este navegador. El enlace no comparte información con otros dispositivos.</div><button class="primary" type="button" data-action="copy">Copiar enlace</button>`,
  };
...
window.addEventListener('storage', async e => { if (event && e.key === `chip-in:event:${event.id}` && !modal) { try { event = await api.getEvent(event.id); render(); } catch (error) { toast(error.message); } } });
const eventId = new URLSearchParams(location.search).get('event');
if (eventId) { try { event = await api.getEvent(eventId); } catch (error) { toast(error.message); } }
render();
